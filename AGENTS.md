...
- **External-process security required:** the marketplace treats ambient-`PATH` execution as a blocker, so every helper must follow this pattern (see `media/CliampPlayer.qml`):
  - Absolute executable identities only. Never rely on ambient `PATH`: no bare command names, no `command -v`, and no bare validator primitives (`stat`, `dirname`, `grep`) inside helpers. Use shell builtins plus absolute paths (`/usr/bin/stat`), shell parameter expansion (`${p%/*}`) instead of `dirname`, and `/usr/bin/env -i` for a closed environment where practical.
