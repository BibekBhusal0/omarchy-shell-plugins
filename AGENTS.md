...
- **External-process security required:** the marketplace treats ambient-`PATH` execution as a blocker, so every helper must follow this pattern (see `media/CliampPlayer.qml`):
  - Absolute executable identities only. Never rely on ambient `PATH`: no bare command names, no `command -v`, and no bare validator primitives (`stat`, `dirname`, `grep`) inside helpers. Use shell builtins plus absolute paths (`/usr/bin/stat`), shell parameter expansion (`${p%/*}`) instead of `dirname`, and `/usr/bin/env -i` for a closed environment where practical. For `Process` helpers that need session access, set `clearEnvironment: true` with an `environment` object holding only a fixed system `PATH` plus the session variables actually required (e.g. `HYPRLAND_INSTANCE_SIGNATURE`, `XDG_RUNTIME_DIR`); pass inherited values through as `null`.
  - Trusted-identity validation before first execution of anything the shell runs automatically: fixed candidate allowlist plus per-component checks without following symlinks (`test -L` plus `stat` per component) requiring root ownership, no symlinks, and no group/other write bits on the file and every ancestor directory. Fail closed when nothing validates.
  - Bounded helpers: enforce the stdout+stderr byte cap per-chunk on arrival before any parsing, a watchdog deadline shorter than the poll interval, SIGKILL plus reap on overflow/timeout, and never overlap runs (skip while the prior helper is still running).
  - Post-parse validation: reject oversized strings and collections, finite-check and clamp numbers.
  - Validate file outputs (backgrounds, downloads) as bounded regular files under allowed locations before consuming them.
