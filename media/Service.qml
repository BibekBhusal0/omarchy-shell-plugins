...
Item {
  ...
  function playerShuffleSupported(player) {
    ...
    if (isCliampMpris(player))
      return cliampPlayer.shuffleSupported;
  ...
  function playerLoopSupported(player) {
    if (!player)
      return false;
    if (isCliampMpris(player))
      return cliampPlayer.loopSupported;
  ...
  function cycleRepeat(targetKey) {
    var player = targetForToggle(targetKey);
    if (!player || !playerLoopSupported(player))
      return false;
    ...
    if (isCliampMpris(player)) {
      ...
      if (nextState === "all")
        nextLoop = MprisLoopState.Playlist;
      else if (nextState === "one")
        nextLoop = MprisLoopState.Track;
